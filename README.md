...
📖 Full documentation, including package architecture and the complete `officequarto.*` option
reference: <https://trekonom.github.io/officequarto/>
...
2. **Scaffold a new project**, pointing at your existing Word document:
   ...
   see `vignette("options", package = "officequarto")` for what each key does), a copy of
   `original.docx`, the officequarto extension under `_extensions/officequarto/`, and one starter
   `.qmd` file — everything needed to render immediately:
   ...
   `officequarto.keep-rendered: false` to `true` (see the options vignette).
...
The full writeup of how the package and bundled extension fit together, and a complete,
field-by-field reference for every `officequarto.*` YAML option, live in two vignettes rather than
here:

- `vignette("architecture", package = "officequarto")` — package layout, what happens on
  `quarto render`, and why there's no manual body write-back step.
- `vignette("options", package = "officequarto")` — style-mapping, lists, tables, figures,
  free-form style-map, footnotes/endnotes, page layout, cross-references, live numbering, style
  pruning, and the full option-reference table.

Both are also published at <https://trekonom.github.io/officequarto/articles/>.
...
- Style-mapping's body-role detection (`officequarto.styles.body`) relies on an allowlist of known
  ...
  gap — see the "Footnotes and endnotes" section of `vignette("options")`); a reference-doc that
  makes Pandoc render *body* text under a not-yet-listed role name also won't be recognized.
...
- Style pruning is unconditional: if content actually uses a Pandoc-added style your `reference-doc`
  ...
  definition is still removed and the content falls back to Word's default formatting — see the
  "Style pruning" section of `vignette("options")`.
