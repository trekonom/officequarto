...
[Caption position](#caption-position) below — easy to miss if you only skim the
...
see [Caption position](#caption-position) below.
...
image paragraph) — see [Table captions](#table-captions) above for the
...
- If `reference-doc` defines its own built-in footnote-text style, Pandoc already reuses that
  ...
  under [Free-form style mapping](#free-form-style-mapping) above. No
...
`officequarto`. Every option lives nested under `format.docx.officequarto` — the "New name"
column below omits that common `officequarto.` prefix's parent path for brevity, but always starts with the
