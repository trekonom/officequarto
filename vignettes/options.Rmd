---
title: "Configuring officequarto"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Configuring officequarto}
...
> See [`dev/spike-notes.md`](https://github.com/trekonom/officequarto/blob/main/dev/spike-notes.md)
...
Everything above is configured via `_quarto.yml`. This one is different: it's an R-session
feature, not an `officequarto.*` YAML option, that lets you use {officer}'s run/paragraph/block
constructors (`ftext()`, `fp_par()`, `run_word_field()`, `block_pour_docx()`, ...) directly as
inline R expressions in a `.qmd`, the same way {officedown} lets you in an `.Rmd`. For example
(shown here without backticks to avoid this vignette itself trying to evaluate it):
